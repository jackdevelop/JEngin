...
function Jplist(plistFileName,format,imageName)
	return function (idx) 
		...
		plistFileName =plistFileName..GameConstants.FileExtensionName_PLIST;
		imageName   = imageName..GameConstants.FileExtensionName_PNG;
		
		if idx=="plist" then return plistFileName
		elseif idx=="image" then return imageName
		elseif idx=="format" then return format 
		else return plistFileName,format,imageName end
	end
end
